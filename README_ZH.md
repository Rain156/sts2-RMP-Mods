...
`0.1.8` 是当前的 Reforge 正式版，已完成《杀戮尖塔2》`v0.107.1` 与 `public-beta v0.108.0` 的兼容验证。两个游戏版本共用同一套 RMP 源码和网络协议，不需要单独安装仅供 `0.108` 使用的版本。
...
> **兼容性说明：** `0.1.8` 已在游戏 `v0.107.1` 与 `public-beta v0.108.0` 上完成兼容检查。同一房间内的所有玩家应使用同一个 RMP 版本。房间容量固定为 16；本地配置只控制难度缩放和 macOS TLS 兼容补丁。
